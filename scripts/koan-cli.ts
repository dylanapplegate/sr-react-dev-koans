 ...
 */
function findKoanDirectory(koanId: string): string {
 ...
 */
function listAvailableKoans(koansDir: string): string {
  try {
    const koanDirs = fs.readdirSync(koansDir)
      .filter(dir => fs.statSync(path.join(koansDir, dir)).isDirectory());
 ...
 */
function displayKoanContent(koanDir: string) {
 ...
 */
function displayUserInstructions(koanDir: string, koanId: string) {
